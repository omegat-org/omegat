...
- Windows installers require InnoSetup with all optional language files.
  See [Build the installer package and test it](93.BuildingInstallerPackage.md)

- Signed Windows installers require osslsigncode.
  See the [Code Signing how-to](92.CodeSigning.md)

- Manual HTML generation requires a container execution environment.
...
runOnJava17 - Launchs app on Java 17
runOnJava21 - Launchs app on Java 21
...
manualZips - Builds ZIP manuals to bundle into application. Requires container runtime.
...
macArmDistZip - Creates Mac distribution for macArm
macArmSignedDistZip
macX64DistZip - Creates Mac distribution for macX64
macX64SignedDistZip
...
testOnJava17 - Runs test cases on Java 17
testOnJava21 - Runs test cases on Java 21
...
You can add command line arguments to Gradle command with `--args="argument value"`.
...
#What is "with intalled structure"?
...
You can build a specific modules jar, by using the `modlue::task` syntax.
