...
# register your plugin artifact to the plugins database

To register your plugin to plugins database and support remote installation, you need to register your plugin jar
to the omegat-plugins project. You can visit the repository at
https://github.com/omegat-org/omegat-plugins

There is `plugins` directory;

   plugins/
      tokyo/northside/omegat/epwing/
      org/omegat/connectors/machinetranslators/microsoft-azure/
      org/exmaple/omegat/plugins-A/
      (group id)/(plugin id)/plugin-name-version.jar

You can fork a project and add directory that has your groupid and project id, and add plugin jar and GnuPG/OpenPGP 
signature file, then send Pull-Request to the project.

These files are checked automatically when raising pull-request. There are two type of check

- Whether jar file has proper MANIFEST.MF with mandatory attributes, such as `Plugin-Name`
- Verify jar.asc signature is signed by a valid signature.

You should ask the project manager to merge your GnuPG/OpenPGP public keyring into
the project keyring/(github account id).gpg file before raising a plugin.jar merge PR.

When successfully merged your request to upload your plugin jar, GitHub actions automatically
retrieve metadata from your jar and compile into JSON database file which OmegaT will download.

It is automatically generated through continuous integration feature.
...
CI tool automatically adds information to allow OmegaT to download and validate the download file.
