...
OmegaT è in grado di riconoscere immediatamente i seguenti formati di file formattati:
  ...
  - Wordfast TXML
  - CSV locale Magento CE
  - Camtasia per Windows
...
Per gli utenti di Linux: OmegaT non lavora con le implementazioni Java free/open-source inserite nelle molteplici distribuzioni Linux (per esempio, Ubuntu), poiché queste non sono aggiornate o sono incomplete. Scaricare e installare 
Java Runtime Environment (JRE) di Sun servendosi del collegamento sopra riportato, oppure scaricare e installare la versione di OmegaT completa di JRE (il pacchetto .tar.gz contrassegnato "Linux").
