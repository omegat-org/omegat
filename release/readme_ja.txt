...
OmegaTは、Free Software Foundationが公開しているGNU General Public License Version 3および(任意の)以降のバージョンのライセンス条項の下で利用可能です。
ライセンスのテキストは、
...
OmegaT の実行には、Java Runtime Environment（JRE）バージョン 11以降があらかじめインストールされている必要があります。
バンドルされたJREパッケージには、現在JRE 17が同梱されています。
...
OmegaTは、お使いのプラットフォーム
やニーズに応じて、いくつかのパッケージ形式で提供されています：

Linux
  - amd64 および aarch64 向けに、deb パッケージと rpm パッケージが用意されています
    。古い32ビット環境はサポートされていません。
  - tar.bz2のアーカイブも利用可能です。

Mac：
  - 公証済みのDMGパッケージが提供されており、Apple Silicon と
    lインテルアーキテクチャをサポートします。

Windows
  - インストーラーEXEファイルは3つのバリエーションで提供されており、1つは
    amd64 JRE、aarch64 JREにセットされたもの、そしてJREのないもの
    (すでにシステムにインストールされているJREを利用します)。32ビット
    JREバンドル版は提供されていません。インストーラープログラムに注意してください
    それ自体は32ビットのIntelバイナリです。これはアーキテクチャには影響しません
    OmegaTやインストールされるJREのことです。

ソースコードパッケージ
  - ソースパッケージには必要なライブラリの依存関係がすべて含まれているため、その
    サイズは比較的大きいです。通常の開発目的では、
    代わりにGitHubからリポジトリをクローンすることを推奨しています。ソースコードパッケージ
    は、インターネット接続が限られた環境でもOmegaTの構築を支援することを
    意図しています。

シンプルなZIPアーカイブ:
  - プレーンなZIPアーカイブも利用可能です。これはLinuxや
    ポータブルパッケージが必要なときにWindowsを使い、自分でJREを用意することもできます。
    また、Mac上で自己公証を行う際にも使用できます。
...
OmegaT .dmgパッケージを開いて、OmegaTアプリケーションを
アプリケーションフォルダ(またはお好きな他の場所)にコピーしてください。
...
DEBまたはRPMパッケージのどちらを使う
-------------------------------------------------------------

あなたのアーキテクチャ(amd64またはaarch64)に合ったパッケージを、
...
インストーラーは、スタートメニュー、デスクトップ、およびクイック起動領域にショートカットを作成できます。[参考] あるいは OmegaT.exe ファイルをマウスの右ボタンでドラッグすれば、ショートカットを自分で作成し、上記のような場所へ配置することもできます。
...
deb または rpm パッケージを使用して OmegaT をインストールした場合は、
デスクトップ環境のアプリケーションメニューから起動するか、次のコマンドを端末から実行してください：
