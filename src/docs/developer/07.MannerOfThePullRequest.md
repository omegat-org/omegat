...
- **State what to address, not why you'd have done it differently.**

If a contribution's design differs from how you personally would have solved the problem, that comparison belongs in your own head, not in the
comment. Say what needs to change and, if useful, why it matters for correctness, maintainability, or scope — not the internal reasoning that
led you to prefer a different design.
