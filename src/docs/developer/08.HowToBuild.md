...
Module projects (those applying the `org.omegat.module` plugin) get the same information from the
`omegatModule` extension instead of the root project, and resolve the bundled jars through its
helpers:
...
dependencies {
    if (file(providedCoreLibsDir).directory) {
        ...
        implementation omegatModule.providedModuleLib('language-ar')
    } else {
        // Download dependencies from configured repositories (Maven Central, etc.)
    }
}
...
Each argument is an artifact name; the helper expands it to `**/<artifact>-*.jar` under
`lib/provided/core` or `lib/provided/module`. Unlike a plain `fileTree`, the helpers fail the build
when a call matches no jar at all, so an incomplete source distribution cannot silently produce a
module without its content. Use them rather than `fileTree` in module build scripts —
`LanguageModuleContentTest` enforces this for the language modules.
