...
A few tests guard interactive code paths against performance regressions,
for example `EditorResizePerformanceTest`. They run as part of the normal
unit test suite and assert wall-clock time with a generous budget.
Timing assertions are sensitive to machine load, so treat a failure as a
hint to re-run and investigate rather than as a hard verdict. On CI runners
(detected through the `CI` or `TF_BUILD` environment variable) they are
skipped entirely.
There is no dedicated benchmark infrastructure (such as JMH) at this time.
