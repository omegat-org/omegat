...
### Excluding repository infrastructure files

Localization projects should include only translatable source files.
Repository infrastructure files, such as GitHub Actions workflows, are not translation content and should not appear in the project file list.

In particular, when a language project maps the `doc_src` repository, exclude the `.github/` directory from the source file selection.
For example:

```xml
<repository type="git" url="https://github.com/OmegaT-L10N/doc_src.git">
    <mapping local="source/" repository="">
       <excludes>.github/**</excludes>
    </mapping>
</repository>
```

If files such as `.github/workflows/sync.yml` appear in an OmegaT language project, update that project's repository mapping or source file selection settings.
Do not translate those files.

Some language teams may also decide not to maintain older manual versions.
In that case, the corresponding manual directories can be excluded in the same mapping.
For example, to exclude the 6.0 and 6.1 manual sources:

```xml
<repository type="git" url="https://github.com/OmegaT-L10N/doc_src.git">
    <mapping local="source/" repository="">
       <excludes>.github/**</excludes>
       <excludes>manuals-6.0/**</excludes>
       <excludes>manual-6.1/**</excludes>
    </mapping>
</repository>
```

Adjust these exclusions for each language project according to the versions that the team wants to maintain.
