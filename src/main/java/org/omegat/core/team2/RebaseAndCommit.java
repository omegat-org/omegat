...
public final class RebaseAndCommit {
    ...
    public static PreparedFileInfo prepare(RemoteRepositoryProvider provider, File projectDir, String path)
        ...
        try {
            ...
            LOGGER.atWarn().logRB("TEAM_BASE_VERSION_LOST", path, currentBaseVersion);
            LOGGER.atDebug().setCause(ex).log("stale base version in prepare");
            return null;
        }
    ...
    private static BaseState resolveBase(PreparedFileInfo prep, RemoteRepositoryProvider provider,
        ...
        try {
            ...
            LOGGER.atWarn().logRB("TEAM_BASE_VERSION_LOST", path, savedVersion);
            LOGGER.atDebug().setCause(ex).log("stale base version in rebaseAndCommit");
