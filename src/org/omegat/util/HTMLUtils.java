...
public final class HTMLUtils {
     ...
     * <p>
     * This method identifies the following BOM types:
     * UTF-16BE (Big Endian), UTF-16LE (Little Endian), and UTF-8.
     * If no BOM is detected, it returns null.
     *
     * @param inputStream the BufferedInputStream to read and detect the BOM from. The stream's position will be reset after detection.
     * @return the Charset corresponding to the detected BOM, or null if no BOM is found.
     ...
     *
     * @param inputStream the BufferedInputStream to read and detect the encoding from. The stream's position will be reset after detection.
     * @param defaultEncoding the default encoding to use if no encoding is specified in the HTML file.
     * @return the Charset corresponding to the detected encoding, or null if no encoding is found.
     * @throws IOException if an I/O error occurs while reading from the stream.
     */
    private static Charset detectEncodingFromContent(BufferedInputStream inputStream, String defaultEncoding) throws IOException {
        inputStream.mark(OConsts.READ_AHEAD_LIMIT);
    ...
    /**
     * Detects the character encoding of a given content by matching it against a series of patterns.
     * <p>
     * Each pattern is used to search for an encoding declaration in the content, and if found, the
     * corresponding charset is returned. If none of the patterns match or an invalid charset is encountered,
     * null is returned.
     *
     * @param content the content in which the character encoding is to be detected
     * @param patterns an array of regular expression patterns used to identify the character encoding declaration
     * @return the detected Charset if a matching pattern is found and a valid encoding is identified, or null if no match or valid encoding is found
     */
