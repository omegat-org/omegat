...
public class TMXReader2 {
    ...
    private static final org.omegat.util.logging.Logger LOGGER = org.omegat.util.logging.Logger
            .getLogger(TMXReader2.class.getName());
    ...
    public void readTMX(File file, final Language sourceLanguage, final Language targetLanguage,
        ...
        LOGGER.logRB("TMXR_INFO_READING_FILE", file.getAbsolutePath());
        ...
        try (InputStream in = getInputStream(file)) {
            ...
            LOGGER.logErrorRB(ex, "TMXR_ERROR_XML_STREAM_ERROR", file.getAbsolutePath());
            throw ex;
        } catch (IOException e) {
        } finally {
            if (xml != null) {
        ...
        if (!allFound) {
            LOGGER.logWarningRB("TMXR_WARNING_SOURCE_NOT_FOUND");
            warningsCount++;
        }
    ...
    protected void parseHeader(StartElement element, final Language sourceLanguage) {
        ...
        // log some details
        LOGGER.logRB("TMXR_INFO_CREATION_TOOL", getAttributeValue(element, "creationtool"));
        LOGGER.logRB("TMXR_INFO_CREATION_TOOL_VERSION", getAttributeValue(element, "creationtoolversion"));
        LOGGER.logRB("TMXR_INFO_SEG_TYPE", getAttributeValue(element, "segtype"));
        LOGGER.logRB("TMXR_INFO_SOURCE_LANG", getAttributeValue(element, "srclang"));
        ...
        if (!sourceLanguage.getLanguage().equalsIgnoreCase(tmxSourceLanguage)) {
            LOGGER.logWarningRB("TMXR_WARNING_INCORRECT_SOURCE_LANG", tmxSourceLanguage, sourceLanguage);
        }

        // give a warning that TMX file will be upgraded to sentence
        // segmentation
        if (isSegmentingEnabled && isParagraphSegtype) {
            LOGGER.logWarningRB("TMXR_WARNING_UPGRADE_SENTSEG");
        }
    }
        ...
        while (true) {
            ...
            case XMLEvent.END_ELEMENT:
                ...
                if (tagN == null) {
                    ...
                    LOGGER.logErrorRB("TMX_ERROR_READING_LEVEL2", e.getLocation().getLineNumber(),
