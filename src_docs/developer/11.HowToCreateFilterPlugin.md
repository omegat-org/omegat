...
OmegaT project publishs `OmegaT.jar` on the Maven Central, so you can add OmegaT as dependency for Gradle/Maven project.

Here is an example of how to configure the plugin project in a Gradle project.
You can use a skeleton project as a project template forked from [plugin-skeleton](https://github.com/omegat-org/plugin-skeleton)
...
And there is a place for comments and for protected parts (a.k.a. tags).
...
comment as a separate argument, which is easier if you don't have other properties.
...
(and the values are also on the comment pane, but layout is not as good as on segment properties pane)
