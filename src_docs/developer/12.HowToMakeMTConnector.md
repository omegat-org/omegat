...
OmegaT can be extended with a plugin. One of the popular categories of the plugin is a Machine Translation (MT) connector.
...
- [NICT TexTra](https://github.com/miurahr/omegat-textra-plugin/releases)
...
a skeleton project. Please go to the [plugin skeleton](https://github.com/omegat-org/plugin-skeleton) and
clicka "Use this template" button to start your project in GitHub.
When you use the codeberg.org forge site, there is also a skeleton at [codeberg.org](https://codeberg.org/miurahr/omegat-plugin-skeleton)
...
There is a main API, `org.omegat.gui.exttrans.IMachineTranslation`.
All MT connectors should implement this interface.
...
tLang language, and returns translation text.
...
When the plugin returns true for the method `isConfigurable`, OmegaT will call `showConfigurationUI` when the user 
clicks a configuration UI button on `Options > Preferences > Machine Translation`.

## Convenience abstract class for plugins

There are two convenience classes for plugins. You are recommended to use `org.omegat.core.machinetranslators.BaseCachedTranslator`.
It implements many necessary things, and you can concentrate on the logic to access your MT engine API. There are only three methods you should override.
...
You can check details at [Jackson Document](https://github.com/FasterXML/jackson-docs)
...
You may be interested in the `get`, `post` and `postJSON` methods.
...
`machinetranslators/aperitium/src/test/java/org/omegat/machinetranslators/aperitium/ApertiumTranslateTest.java` 

You can also see a plugin project Azure-translate-plugin to learn how to write a test.
`TestMicrosoftTranslatorAzure.java` implements a case to test HTTP requests and responses.
