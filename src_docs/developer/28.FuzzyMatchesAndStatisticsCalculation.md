...
| **Exact Match** | In statistics: a segment already translated in the current project. In matching: a 100% identical source. |
| **100% Bucket** | A fuzzy match category representing 100% similarity before penalties; distinct from "Exact Match" in statistics reports. |
...
5.  **Output**: Data is passed to `IStatsConsumer` for UI display or file export (Text/XML/JSON).
...
- **Determinism**: Matches are sorted by score, then by specific tie-break rules to ensure stable presentation.

### 3.4 Code Structure

| Group | Path / Responsibility | Why you'd go there |
| :--- | :--- | :--- |
| **API / SPI** | `org.omegat.core.matching.ISimilarityCalculator`, `org.omegat.core.statistics.spi.*` | To implement custom similarity algorithms or new statistics output formats. |
...
| **Statistics Core**| `org.omegat.core.statistics.Calc*Statistics`, `dso.*` | To change how segments/words are counted or how buckets are defined. |
...
### 4.3 Tie-break Rules
If two matches have the same score, the order is determined by:
1.  Higher raw score (before penalties).
2.  Presence of translation.
3.  Alphabetical order of the source text.
...
- **Tags & Protected Parts**:
    ...
    - Protected parts are replaced with a special internal character (`\u0000`) for matching to ensure they are treated as atomic units.
- **Removed-text Handling**: Conceptual logic that ignores specific parts of the segment to improve matches for boilerplate text.
...
| Setting (Internal Name) | UI Label | Use Case | Default |
| :--- | :--- | :--- | :--- |
| `minmatch_threshold` | Minimum similarity | Filter out low-quality matches. | 30% |
| `stat_counting_protected_text` | Count protected text | Include/exclude protected parts in word counts. | Yes |
| `stat_counting_custom_tags` | Count custom tags | Treat custom tags as words. | Yes |
| `penalty_foreign_matches` | Foreign match penalty | Penalize TMs from different locales. | 30% |
| `matches_stemming_full` | Use stemming | Enable/disable stemmed matching pass. | Yes |

## 9. Performance & threading

- **Threading**:
    - Match finding runs in `FindMatchesThread` (one per request).
    - Statistics calculation runs in `CalcMatchStatistics` (can be parallelized across CPUs via `omegat.stats.parallel`).
...
- **Cancellation**: Long-running statistics tasks frequently check `isInterrupted()` to remain responsive to user cancellation.

## 10. Output & extensibility

- **Display**: The Matches pane uses HTML/Swing for display.
- **File Export**: `project_stats.txt`, `project_stats.xml`, and `project_stats.json` are generated in the project root.
- **Extending**:
...
#### Example 1: Multi-pass Scoring
- **Source**: "The quick brown fox"
- **TM**: "A quick brown fox"
- **Pass 1 (Stemmed)**: Both might normalize to "quick brown fox". Score: 100%.
- **Pass 3 (Verbatim)**: "The" vs "A" mismatch. Score: 75% (3/4 tokens).
- **Result**: The system selects the highest valid score (100%) but applies penalties if Pass 3 reveals tag/punctuation differences.

#### Example 2: Penalties
- **Source**: `Click <a1>here</a1>`
- **TM**: `Click here` (100% text match)
- **Tag Penalty**: 5% reduction.
- **Final Score**: 95%.

#### Example 3: Repetitions vs Exact
- Segment A (translated)
- Segment B (identical to A, untranslated)
- **Statistics**: A is "Exact Match", B is "Repetition".
- If both are untranslated, the first encountered is "Unique", the second is "Repetition".
