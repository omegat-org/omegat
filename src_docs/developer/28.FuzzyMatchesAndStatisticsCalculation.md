...
| **Context**                | The context, which contains id, before and after segements, in which the source segment appears.            |
...
These bucket is defined in `MatchStatCounts`, counted in `MatchStatCount#addForPersent` and `MatchStatCount#addExact` methods.
It is used to calculate max similarity statistics in `CalcMatchStatistics#calcSimilarity`.
