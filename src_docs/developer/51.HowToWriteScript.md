...
The `SegmentExportImport` feature allows external tools and scripts to interact with the currently active segment via files in the **OmegaT Configuration Script Directory** (`<configDir>/script`).

-   **`source.txt`**: Contains the source text of the current segment.
-   **`target.txt`**: Contains the target text (translation) of the current segment.
-   **`selection.txt`**: Contains the currently selected text in the editor.
-   **`import.txt`**: If a script writes to this file, OmegaT will automatically read its content and replace the current segment's translation with it.
