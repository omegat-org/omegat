...
- **Signal Noise ratio**: After enabling the checks, builds on the development branch (e.g., 6.1 Beta) surface many compiler warnings (Error Prone/NullAway, SpotBugs). These can be perceived as "errors" even though the build succeeds. This discrepancy with `releases/6.0` (where such checks are not enabled) can create [confusion among contributors](https://sourceforge.net/p/omegat/mailman/message/59262237/).
...
- ["Errors with the Task :compileJava" at development ML(20251121)](https://sourceforge.net/p/omegat/mailman/message/59262237/)
