...
- Not always valid TMX 1.1 or 1.4b,
- Sometimes misinterpreted by other TMX consumers due to OmegaT-specific tag encoding.
...
- The `<!DOCTYPE>` declaration in `project_save.tmx` is misleading for users opening files in validating editors like OxygenXML
...
- Emphasized that `project_save.tmx` is primarily **storage**, not an exchange format
- Users have "rerouted" its usage from initial design goals, making format changes more difficult
- Need to balance original storage purpose with evolved user expectations
...
3. Research modern XML framework approaches for future writing

#### Pros
- Eliminates misleading DOCTYPE while maintaining TMX structure
...
#### Cons
- Still requires consensus on missing DOCTYPE interpretation
- May need careful user communication about format changes
