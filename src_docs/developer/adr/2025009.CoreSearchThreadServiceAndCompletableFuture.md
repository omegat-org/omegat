...
2. **Missing Cancellation**: 2 of 5 panes do not have a cancel mechanism
3. **Thread Accumulation**: Each entry activation creates new threads
4. **No Completion Tracking**: No way to wait for completion (essential for modern testing)
...
We will add a common cancelation mechanism during search/find/fetch operation in base `EntryInfoThreadPane` class.
Consolidate all five panes to use the same architecture.
