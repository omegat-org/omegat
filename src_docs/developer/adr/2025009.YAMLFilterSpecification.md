...
- **Implemented** - Date: 2026-03-28
    - Path-based include/exclude rules implemented with wildcard support.
...
- Basic extraction and advanced selection by path (include/exclude) with wildcard support (`*` and `**`).
...
- Include/Exclude Rules: Users can specify path-based include and exclude patterns to filter translation targets.
    - Path: Uses `/` as a separator for nested keys and `[n]` for sequence indices.
    - Wildcards:
        - `*` matches a single path segment.
        - `**` matches zero or more path segments.
    - Priority: If include patterns are specified, only paths matching an include pattern are translated. If no include patterns are specified, all paths are translated except those matching an exclude pattern.
