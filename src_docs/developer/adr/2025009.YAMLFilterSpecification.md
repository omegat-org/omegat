...
- The project already depends on Jackson’s YAML binding (via `com.fasterxml.jackson.dataformat.yaml.YAMLFactory`). There is working sample code (`OmegaTTipOfTheDayModel`) that demonstrates reading YAML using `ObjectMapper` + `YAMLFactory` and `mapper.readTree(is)`.
...
- Source directory: `src/omegat/filters2/text/yaml/`
- Class: `org.omegat.filters2.text.yaml.YamlFilter`

#### Plugin Requirements
- The filter must be distributed/loaded as a plugin. The class must expose:
  - `public static void loadPlugins()` — register the filter with OmegaT’s filter framework.
  - `public static void unloadPlugins()` — unregister/cleanup as needed.
- Internal registration: add an entry into `Plugins.properties` at the project root to ensure the filter is loaded in the internal build. The exact key naming follows existing conventions for internal plugins; example style:
  - Example (illustrative):
    ```
    # YAML filter plugin (internal)
    plugin.filter.yaml=org.omegat.filters2.text.yaml.YamlFilter
    ```
  - Use the established key format used by other internal filters in this file.

For guidance on building external plugins and manifests, see:
- `src_docs/developer/11.HowToCreateFilterPlugin.md`
- `src_docs/developer/19.PluginManifest.md`

#### Dependencies
- Jackson YAML binding
  - Parser/Mapper: `com.fasterxml.jackson.databind.ObjectMapper`
  - YAML Factory: `com.fasterxml.jackson.dataformat.yaml.YAMLFactory`
- Initialization example: mirrored from `OmegaTTipOfTheDayModel`:
  ```java
...
- Sequences: extract elements that are string scalars; ignore non-string elements.
...
- Preserving comments/formatting via a different library: deferred; increased complexity outweighs benefits for v1.
...
- `src_docs/developer/11.HowToCreateFilterPlugin.md`
- `src_docs/developer/19.PluginManifest.md`
- Example usage of Jackson YAML: `tipoftheday/src/main/java/org/omegat/gui/tipoftheday/OmegaTTipOfTheDayModel.java`
