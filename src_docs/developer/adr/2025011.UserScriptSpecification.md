...
- Adding other languages (e.g., Python, Ruby) is possible by adding the corresponding scripting engine JARs to the OmegaT `lib` or `plugins` directory.
...
3.  **Script Metadata and Localization**:
    - Script names and descriptions can be localized using `.properties` files in a `properties/` subdirectory relative to the script's location.
    - Alternatively, metadata can be embedded in the script file using the format:
      ```
      // :name = My Script
