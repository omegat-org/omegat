...
2.  **Source Term Length**: If enabled (`GLOSSARY_SORT_BY_SRC_LENGTH`), longer source terms are prioritized. (*1)
...
Note(*1): In OmegaT 6.0.0 and earlier versions, sorting by source term length was always performed.
...
- **Stemming**: Optional stemming is supported during tokenization if configured in preferences.
