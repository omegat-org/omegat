...
public class Xliff2FilterTest extends org.omegat.filters.TestFilterBase {
    ...
    public void testParse() throws Exception {
        List<String> entries = parse(new Xliff2Filter(), "src/test/resources/data/filters4/xliff2/ex.9.5.xlf");
    ...
    public void testBilingual() throws Exception {
        ...
        parse2(new Xliff2Filter(), "src/test/resources/data/filters4/xliff2/ex.9.5.xlf", result, legacy);
        assertEquals("<t0>Oiseaux de haute altitude", result.get("<t0>High Altitude Birds"));
    }
    
    @Test
    public void testKey() throws Exception {
        List<ParsedEntry> entries = parse3(new Xliff2Filter(), "src/test/resources/data/filters4/xliff2/ex.9.5.xlf",
            java.util.Collections.emptyMap());
        ParsedEntry firstEntry = entries.get(0);
        Assert.assertEquals("1", firstEntry.id); // <segment> has no id, not mandatory
        Assert.assertEquals("//groups/N65541xdocument/N65541bxmarksection-1/title-2", firstEntry.path);
        ParsedEntry secondEntry = entries.get(2);
        Assert.assertEquals("<t0>High Altitude Birds", secondEntry.source);
        Assert.assertEquals("<t0>Oiseaux de haute altitude", secondEntry.translation);
    }

    @Test
    public void testTranslation() throws Exception {
        Xliff2Filter filter = new Xliff2Filter();
        filter.translateFile(new File("src/test/resources/data/filters4/xliff2/ex.9.5.xlf"),
        ...
        Assert.assertEquals("<t0>Oiseaux de haute altitude", entries.get(2).translation);
        // entry translated in the callback, not in the source file
        Assert.assertEquals("Oiseaux en Oregon", entries.get(0).translation);
    }
