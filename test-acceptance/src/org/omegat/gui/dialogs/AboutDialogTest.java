...
public class AboutDialogTest extends TestCoreGUI {
    ...
    public void testAboutDialog() throws InterruptedException {
        ...
        // String aboutMessage = window.dialog(AboutDialog.DIALOG_NAME).textBox().text();
        String javaVersion =
                window.dialog(AboutDialog.DIALOG_NAME).label(AboutDialog.JAVA_VERSION_LABEL_NAME).text();
        ...
        assertTrue(window.dialog(LicenseDialog.DIALOG_NAME).textBox(LicenseDialog.LICENSE_TEXT_PANE_NAME).text()
                .contains("========="));
