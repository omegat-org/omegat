...
public class Xliff1FilterTest extends org.omegat.filters.TestFilterBase {
    ...
    @Test // checks that file wihout ID will fail (REQUIRED in the
          // specification)
    public void testParseMissingId() throws Exception {
        Exception exception = assertThrows(TranslationException.class, () -> {
            // Try with a file from filters3 : they are not conform to
            // specification so it should fail
            parse(new Xliff1Filter(), "test/data/filters/xliff/filters3/file-XLIFFFilter.xlf");
        });
        System.err.println("Parsing file where trans-unit/@id is missing returns: " + exception.getClass()
    ...
    public void testKey() throws Exception {
        List<ParsedEntry> entries = parse3(new Xliff1Filter(),
                "test/data/filters/xliff/filters4-xliff1/en-xx.xlf", java.util.Collections.emptyMap());
        ParsedEntry firstEntry = entries.get(0);
        assertEquals(firstEntry.id, "example_01");
        assertEquals(firstEntry.path, "//interface.po"); // path is built with
                                                         // file name
        ...
        assertEquals("Added_1", lastEntry.id); // next entry Added_2 is ignored
                                               // as translate=no
        assertEquals("//interface.po/Group 3", lastEntry.path); // path is built
                                                                // with file
                                                                // name and
                                                                // groups
    }
    ...
    public void testTranslation() throws Exception {
        ...
        filter.translateFile(new File("test/data/filters/xliff/filters4-xliff1/en-xx.xlf"), outFile,
    ...
    public void testTranslationRFE1506() throws Exception {
        ...
        // org.omegat.filters.XLIFFFilterTest.checkXLiffTranslationRFE1506(filter,
