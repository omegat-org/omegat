...
public class Xliff1FilterTest extends org.omegat.filters.TestFilterBase {
    ...
    public void testKey() throws Exception {
        ...
        Assert.assertEquals(firstEntry.path, "//interface.po");  // path is built with file name
        ParsedEntry secondEntry = entries.get(1);
        Assert.assertEquals("foo", secondEntry.source);
        Assert.assertEquals("bar", secondEntry.translation);
        ParsedEntry lastEntry = entries.get(entries.size() - 1);
        Assert.assertEquals("Added_1", lastEntry.id);  // next entry Added_2 is ignored as translate=no
        Assert.assertEquals("//interface.po/Group 3", lastEntry.path);  // path is built with file name and groups
    }
    ...
    public void testTranslation() throws Exception {
        ...
        Assert.assertEquals("bar", entries.get(1).translation);
        // entry translated in the callback, not in the source file
        Assert.assertEquals("Devrait traduire dans le r\u00E9sultat.", entries.get(2).translation);
    }
